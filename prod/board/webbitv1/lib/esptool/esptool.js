  ...
  class ESP extends WebSerial {
    ...
    uploadStub() {
      return new Promise(async (resolve) => {
        ...
        for (let field of ["text", "data"]) {
          await fetch(
            `board/webbitv1/lib/esptool/stub/${this.chip}/${field}.stub`
          )
            .then((res) => {
        ...
        await this.req(ESP_MEM_END, {
          executeFlag: 0,
          entryAddress: ESPInfo[this.chip].ENTRY_ADDR,
        });
    ...
    writeFlash(address, url) {
      ...
      return new Promise(async (resolve) => {
        ...
        // compsize
        let num_blocks = Math.floor(
          (size + FLASH_WRITE_SIZE) / FLASH_WRITE_SIZE
        );
        ...
        while (image.byteLength > 0) {
          ...
          var percent = Math.floor((100 * (seq + 1)) / num_blocks);
          ESP.console.log(`Writing at 0x${addressIndex}... (${percent}%)`);
