const baseUrl = "https://account.webduino.io";
...
const logout = () => {
    // https://account.webduino.io/zh-TW/log-in?grant_type=authorization_code&response_type=code&client_id=thlp6rstwsl4iqa4&redirect_uri=http://localhost.webduino.io:8080/api/v1/auth/callback
    const locale = getCookie("NEXT_LOCALE") || "";
    const url = new URL(`${baseUrl}${locale ? `/${locale}` : ""}/log-in`);
    url.searchParams.append("grant_type", "authorization_code");
    url.searchParams.append("response_type", "code");
    url.searchParams.append("client_id", oauthClientID);
    url.searchParams.append("redirect_uri", `https://chat.webduino.io/api/v1/auth/callback`);
