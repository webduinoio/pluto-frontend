...
function actorDefault(msg, isEnd) {
  try {
    if (isEnd) {
      ...
      var info = parseURL(parent.location.href);
      if (info["actor"] == "wbitv1") {
        parent.Main.coms["deploy_v1"].setEnable(true);
      }
      if (info["actor"] == "wbit") {
        parent.Main.coms["deploy_v2"].setEnable(true);
      }
      gpt.done();
      gpt.wholeMsg = wholeMsg;
      parent.flow.done();
...
async function init() {
  ...
  gpt.promptCallback(function (prompt) {
    ...
    parent.Main.coms["deploy_v1"].setEnable(false);
    parent.Main.coms["deploy_v2"].setEnable(false);
